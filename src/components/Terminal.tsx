...
export function Terminal({ className, commands = [] }: TerminalProps) {
    ...
    return (
        <div className={cn("w-full rounded-lg overflow-hidden border border-border bg-[#1e293b] dark:bg-[#0f172a] shadow-2xl font-mono text-sm", className)}>
            {/* Terminal Header */}
            <div className="flex items-center gap-2 bg-white/5 px-4 py-2 border-b border-white/5">
                ...
                <div className="flex-1 text-center text-xs text-white/30">sre-portfolio ~ bash</div>
            </div>

            {/* Terminal Body */}
            <div
                ref={containerRef}
                className="p-4 h-auto min-h-[160px] overflow-y-auto text-white/80 space-y-2 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent"
            >
                {lines.map((line, i) => (
                    <motion.div
                        key={i}
